...
@app.route('/')
def index():
    urls = {'За неделю': url_for('week'),
            'За месяц': url_for('month')}
...
#    for i in range(3313356, 3329373):
#        pageUrl = url + str(i)
...
def week():
#    url = 'https://www.kommersant.ru/doc/'
#    for i in range(3324239, 3329373):
#        pageUrl = url + str(i)
#        download_page(pageUrl)
#    text()
#    d_nouns = counting() #получаем словарь из существительных и их частот
...
def download_page(pageUrl):
    arrUrl = [] # массив, в котором лежат все ссылки на статьи
    try:
